(() => {
  ...
  const MASTER_PASSWORD = "Hub69440510**";
...
function init() {
    ...
    // ============================================================
    // ACESSO DO CLIENTE POR NOME
    // Exemplo:
    // ?cliente=João%20da%20Silva
    // ============================================================
    ...
    if (clienteParam) {
      ...
      // Normaliza o texto para aceitar diferenças de maiúsculas,
      ...
      const proceedClientReadOnly = () => {
        // Ativa o modo cliente.
        setClientMode(true);

        // Abre o projeto.
        openProject(clientData.id);

        // Garantia extra: esconder ferramentas administrativas.
      ...
      // Se o projeto tiver senha própria, pede a senha do cliente.
        ...
        openPasswordModal({
          ...
          onSuccess: (value) => {
            if (value === clientData.clientPassword) {
              ...
              // A senha mestre pode abrir o projeto,
              // mas o acesso continua sendo SOMENTE LEITURA.
              closePasswordModal();
              proceedClientReadOnly();
