(() => {
  ...
  const MASTER_PASSWORD = "menche-2024";
  ...
  let designerUnlocked = localStorage.getItem(DESIGNER_KEY) === "1";
  ...
  function clientFilesHTML(files) {
    if (!files.length) return '<div class="file-empty">Nenhum arquivo nesta etapa.</div>';
    return files
      .map((f) => {
        const isImg = f.type && f.type.startsWith("image/");
        const thumb = isImg ? `<img src="${f.dataUrl}" alt="${escapeHTML(f.name)}" />` : ICONS.fileDoc;
  ...
  function renderSidebar() {
    ...
    const navStages = fullAccess ? STAGES : STAGES.filter((s) => s.id !== "contratos");
  ...
  function renderStageClient(project, stage) {
    ...
    if (stage.special === "contracts") {
      container.innerHTML = header + `
        <div class="panel">
          <h3>${ICONS.contratos} Contratos</h3>
          <div class="file-empty">Conteúdo confidencial — contratos não são exibidos no modo cliente.</div>
        </div>`;
      return;
    }
  ...
  function renderContracts(project) {
    ...
    $("#btnAddContract").addEventListener("click", async () => {
      ...
      let item;
      if (isLink) {
        item = { id: uid(), name, kind: "link", value: normalizeUrl(link) };
      } else {
        ...
        const dataUrl = await readFileAsDataUrl(file);
        item = { id: uid(), name, kind: "file", value: dataUrl, fileName: file.name, fileType: file.type };
      }
    ...
    $$("#contractList .file-remove").forEach((btn) => {
      btn.addEventListener("click", () => {
        const id = btn.closest(".contract-item").dataset.id;
        project.contracts = project.contracts.filter((c) => c.id !== id);
        if (saveProjects()) renderContracts(project);
      });
    });
  }
  ...
  function contractListHTML(contracts) {
    return contracts
      .map((c) => {
        ...
        const sub = c.kind === "link" ? c.value : `${c.fileName} · ${c.fileType || "arquivo"}`;
        return `
          <div class="contract-item" data-id="${c.id}">
            ...
            <a class="file-open" href="${escapeHTML(c.value)}" target="_blank" rel="noopener">Abrir</a>
            <button class="file-remove" title="Remover documento">✕</button>
          </div>`;
      })
      .join("");
  }
  ...
  function init() {
    ...
    const clientData =
      window.__CLIENT_PROJECT__ ||
      (() => {
        const id = params.get("projeto") || params.get("cliente");
        const key = params.get("chave");
        if (!id) return null;
        const list = loadProjects() || [];
        if (key) return list.find((p) => p.id === id && p.shareKey === key) || null;
        return list.find((p) => p.id === id) || null;
      })();

    if (clientData) {
      projects = [clientData];
      const requiresPassword = clientData.clientPassword && !designerUnlocked;
      if (requiresPassword) {
        openPasswordModal({
    ...
    } else if (params.get("projeto") || params.get("cliente")) {
      showClientError();
    } else if (designerUnlocked) {
